...
export default class News extends Component {
    ...
    async componentDidMount(){ 
     ...
     handleNextClick = async ()=>{
        ...
        fetchMoreData = async () => {  
            this.setState({page: this.state.page + 1})
            const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=761fd8c860b14b9a84f7eaae40ee6827&page=${this.state.page}&pageSize=${this.props.pageSize}`;
            let data = await fetch(url);
            let parsedData = await data.json()
            this.setState({
